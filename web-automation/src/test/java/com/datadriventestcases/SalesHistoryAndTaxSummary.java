...
public class SalesHistoryAndTaxSummary extends BaseTest {
	...
	public void initializeEnvironment(String excelName) {
		...
		excelMethods.setSheetName("SalesHistoryAndTax");
	...
	public void salesHistoryAndTaxSummary(Map<String,String> data)  {
		if (data.get("Active").equalsIgnoreCase("y") || data.get("Active").equalsIgnoreCase("yes")){
			...
			objSavedSearchVIew.verifySaleHistoryAndTaxSummaryIsDisplayed(data.get("PropertyState"),data.get("PropertyNameValue"));
		    objLoginPageView.logoutFromTheApplication();
		}else{
